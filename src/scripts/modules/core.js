...
const WINDOW_NAMESPACE_TOOLBAR_LOADER = "toolkitToolbarLoader",
  ...
  // 1. Check the environment name
  isProd = import.meta.env.VITE_APP_ENV === "production",
  minifiedString = isProd ? ".min" : "",
  
  // 2. Get host and port details dynamically
  serverHost = import.meta.env.VITE_SERVER_HOST,
  serverPort = import.meta.env.VITE_SERVER_PORT,
  
  // 3. Construct the clean URL string
  portString = (!isProd && serverPort) ? `:${serverPort}` : "",
  URL_SCRIPT_TOOLBAR = `//${serverHost}${portString}/toolkit.toolbar${minifiedString}.js`;
